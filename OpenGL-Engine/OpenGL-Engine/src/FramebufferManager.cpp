...
void FramebufferManager::generateGBuffer() {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT7, GL_TEXTURE_2D, gTexCoords, 0);
    ...
    const int nAttachTexture = 8;
    std::vector<unsigned int> attachments;
    for (int i = 0; i < nAttachTexture; i++) {
        attachments.push_back(GL_COLOR_ATTACHMENT0 + i);
    }
    glDrawBuffers(nAttachTexture, &attachments.at(0));
...
void FramebufferManager::genScreenSpaceAmbientOcclusion() {
    ...
    for (int i = 0; i < 64; i++) {
        ...
        scale = interpolate::lerp(0.1f, 1.0f, scale * scale);
        sample *= scale;
...
void FramebufferManager::setSSAOShader(Shader* p_SSAOShader, Shader* p_SSAOBlurShader) {
    SSAOShader = p_SSAOShader;
    SSAOBlurShader = p_SSAOBlurShader;
}

void FramebufferManager::drawSSAO(glm::mat4 projection, glm::mat4 view) {
...
void FramebufferManager::SSAOBlur() {
...
void FramebufferManager::setBloomShader(Shader* p_GaussianBlurShader, Shader* p_BloomResultShader) {
    bloomGaussianBlurShader = p_GaussianBlurShader;
    bloomResultShader = p_BloomResultShader;
}

void FramebufferManager::genBloomBuffer() {
...
void FramebufferManager::twoPassGaussianBlur() {
    bool first_iteration = true;
    unsigned int amount = 10;
    bloomGaussianBlurShader->use();
    bloomGaussianBlurShader->setInt("brightnessBuffer", 0);
    for (unsigned int i = 0; i < amount; i++) {
        glBindFramebuffer(GL_FRAMEBUFFER, pingpongFBO[horizontal]);
        bloomGaussianBlurShader->setBool("horizontal", horizontal);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, first_iteration ? brightnessBuffer : pingpongBuffer[!horizontal]);  // bind texture of other framebuffer (or scene if first iteration)
        renderQuad();
...
void FramebufferManager::bloomResult(bool &bloom, float &exposure){
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    bloomResultShader->use();
    bloomResultShader->setInt("scene", 0);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, colorBuffer);
    bloomResultShader->setInt("bloomBlur", 1);
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, pingpongBuffer[!horizontal]);
    bloomResultShader->setBool("bloom", bloom);
    bloomResultShader->setFloat("exposure", exposure);
    renderQuad();
}
...
void FramebufferManager::IrradianceMapping(Shader *irradianceShader, unsigned int envCubemap, int width, int height) {
    ...
    for (unsigned int i = 0; i < 6; ++i) {
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB, convWidth, convHeight, 0, GL_RGB, GL_FLOAT, nullptr);
    }
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, captureFBO);
    glBindRenderbuffer(GL_RENDERBUFFER, captureRBO);
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, convWidth, convHeight);
    ...
    glViewport(0, 0, convWidth, convHeight); // don't forget to configure the viewport to the capture dimensions.
    glBindFramebuffer(GL_FRAMEBUFFER, captureFBO);
    for (unsigned int i = 0; i < 6; ++i)
    {
    ...
    mappers["irradianceMap"] = irradianceMap;
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
}

void FramebufferManager::PreFilterMapping(Shader *preFilterShader, unsigned int envCubemap, int width, int height) {

    glGenTextures(1, &preFilterMap);
    glBindTexture(GL_TEXTURE_CUBE_MAP, preFilterMap);
    for (unsigned int i = 0; i < 6; ++i)
    {
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB, 128, 128, 0, GL_RGB, GL_FLOAT, nullptr);
    }
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
    ...
    mappers["preFilterMap"] = preFilterMap;
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
}

void FramebufferManager::BrdfLUT(Shader *LUTShader, int width, int height) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RG, width, height, 0, GL_RG, GL_FLOAT, 0);
...
unsigned int FramebufferManager::combineTexture(Shader *shader, std::map<std::string, unsigned int>& pTexture, int width, int height) {
    ...
    for (auto& tex : pTexture) {
        id++;
        if (tex.first == "occlusionMap") {
            shader->setBool("useOcclusionMapping", (tex.second != 0));
        }
        if (tex.first == "metallicMap") {
            shader->setBool("useMetallicMapping", (tex.second != 0));
        }
...
void FramebufferManager::renderQuad() {
...
void FramebufferManager::draw(Shader* shader) {
    ...
    glClearColor(0.2f, 0.2f, 0.2f, 1.0f); // set clear color to white (not really necessary actually, since we won't be able to see behind the quad anyways)
    glClear(GL_COLOR_BUFFER_BIT);
